...
app.controller("controller_employ_comment", function($scope:any, $http:any){
    $scope.curPage = 0
    $scope.danhGia = {}
    $scope.APIData = {
        "dg":[],
    ...
    $scope.pageSize = function(){
        if ($scope.APIData.dg.length < 5)
            ...
            if($scope.APIData.dg.length % 5 == 0)
                return $scope.APIData.dg.length / 5
            else
                if(Math.round($scope.APIData.dg.length / 5) < $scope.APIData.dg.length / 5)
    ...
    $scope.loadPage = function(){
        ...
        for(var i = $scope.curPage*5 ; i < $scope.curPage*5 + 5 ; i++){
            if(i == $scope.APIData.dg.length) break
    ...
    $scope.pheDuyet = function(stt:number){
        $http.put(url+"accept/"+stt)
        .then((resp:any) => {
            alert("Phê duyệt đánh giá thành công")
            if(($scope.APIData.dg.length - 1) % 5 == 0) $scope.curPage = $scope.curPage - 1
            $scope.loading()
        })
    }
    $scope.huyBo = function(stt:number){
...
app.controller("controller_employ_not_order", function($scope:any, $http:any){
    const url = "http://localhost:8080/skyPhoneEmploy/order/";
    $scope.curPage = 0
    $scope.notDoneOrder = {}
    $scope.APIData = {
        "hd" : {}
    ...
    $scope.pageSize = function(){
        if ($scope.APIData.hd.length < 3)
            ...
            if($scope.APIData.hd.length % 3 == 0)
                return $scope.APIData.hd.length / 3
            else
                if(Math.round($scope.APIData.hd.length / 3) < $scope.APIData.hd.length / 3)
    ...
    $scope.loadPage = function(){
        ...
        for(var i = $scope.curPage*3 ; i < $scope.curPage*3 + 3 ; i++){
            if(i == $scope.APIData.hd.length) break
...
app.controller("controller_employ_order", function($scope:any, $http:any){
    const url = "http://localhost:8080/skyPhoneEmploy/order/";
    $scope.curPage = 0
    $scope.doneOrder = {}
    $scope.APIData = {
